...
class Lexer:
    ...
    def __new_token(self, tt: TokenType, literal: Any) -> Token:
        return Token(type=tt, literal = literal, line_no=self.lin_no, position = self.position)
    ...
    def __read_number(self) -> Token:
        ...
        while self.current_char is not None and (self.__is_digit(self.current_char) or self.current_char == '.'):
            ...
            if dot_count > 1:
                print("Too many decimals in number on line {self.lin_no}, position {self.position}")
